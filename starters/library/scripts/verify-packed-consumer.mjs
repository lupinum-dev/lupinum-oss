...
for (const manager of ['npm', 'pnpm']) {
  ...
  try {
    ...
    const verify = spawnSync(process.execPath, ['--input-type=module', '--eval', `for (const name of ${JSON.stringify(packageNames)}) { const mod = await import(name); if (mod.greet('Lupinum') !== 'Hello, Lupinum.') throw new Error('Unexpected public API.') }`], {
      cwd: consumer,
      encoding: 'utf8',
      timeout: 30000,
    })
