...
- [ ] The protected workflow verified that the bootstrap is the sole package version.
- [ ] The protected workflow derived bootstrap state without a package list or override.
- [ ] The GitHub release names every package that used bootstrap mode.

## Trusted publishing

- [ ] The trusted publisher uses the exact repository.
- [ ] The trusted publisher targets the repository's exact maintained workflow file.
- [ ] The environment is `npm`.
- [ ] The trusted publisher permits direct publication.
- [ ] The trust record was listed and checked.
- [ ] Package publishing access requires 2FA and disallows bypass tokens.
- [ ] No `NPM_TOKEN` exists in the repository, organization, or environment.

## Public record

- [ ] The protected workflow found the successful artifact for the exact current `main` commit.
- [ ] The Git tag points to the release commit.
- [ ] The GitHub release uses the correct changelog section.
- [ ] A workflow retry rejects different bytes and accepts only the exact certified tarball.
